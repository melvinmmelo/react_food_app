...
function CreateContainer() {
  ...
  const fetchData = async () => {
    await getAllFoodItems().then((data) => {
      dispatch({
        type: actionType.SET_FOOD_ITMES,
        foodItems: data,
      });
    });
  };
    ...
    <div className="w-full min-h-screen flex items-center justify-center">
      <div className="w-[90%] md:w-[75%] border border-gray-500 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
        ...
        <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
          <MdFastfood className="text-xl text-ggray-700" />
          <input
            type="text"
            required
            value={title}
              ...
              {!imageAsset ? (
                ...
                <>
                  <div className="relative h-full">
                    ...
                    <button
                      type="button"
                      className="absolute bottom-3 right-3 p-3 rounded-full outline-none hover:shadow-md duration-500 transition-all ease-in-out bg-red-700"
                      onClick={deleteImage}
                    >
